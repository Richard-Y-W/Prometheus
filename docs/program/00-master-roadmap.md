...
- **01C — Safe evidence acquisition — next, unstarted:** preserve source bytes, add sandboxed deterministic parsers and optional candidate extraction, and retain explicit licensing and review state.
- **01D — End-to-end review and reproduction:** complete portable project manifests, packaged Windows end-to-end acceptance, and broader reopen/reproduction tests.

Program 01C is the current gate but has not started. The former v1 record remains [historical evidence](01-trust-kernel/01a-integrity-and-contracts.md), and its mutation/export endpoints are retired under the [migration guide](../migration/program-01a-v1-to-v2.md). Program 01B proved that reviewed values, rather than production fixture constants, drive one bounded C++ result and replay exactly under the recorded numeric identity. That result is not arbitrary project verification, physical validation, universal intake, or external solver execution; Programs 01C through 10 remain necessary for the broader product goal.
