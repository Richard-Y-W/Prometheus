...
PreparedV3Sample prepare_v3_sample(
  ...
  const auto setupVersion = setupJson.value("schema_version", "");
  const bool recognizedSetup =
      (setupVersion == "2.0.0" &&
...
StructuralSetup deserialize_setup(const std::string &setupBytes,
  ...
  if (legacyV2 &&
      !exact_keys(root.at("requirement"),
                  {"displacement_limit_m", "von_mises_limit_pa",
                   "source_or_exploratory_rationale",
                   "displacement_limit_basis", "von_mises_limit_basis",
                   "reviewed"}))
  ...
  if (legacyV2) {
    ...
    const auto reviewed = json_bool(requirement, "reviewed");
    if (const auto limit =
      ...
      setup.requirements.push_back(
          {.quantity = RequirementQuantity::displacement,
           ...
           .limit_value = *limit,
           .unit = "m",
           .applicability = "legacy reviewed setup applicability",
    ...
    for (const auto &requirement : root.at("requirements")) {
      if (!exact_keys(requirement,
                      {"quantity", "other_quantity_description", "comparator",
                       "limit_value", "unit", "applicability", "criticality",
                       "source_or_exploratory_rationale", "reviewed",
                       "limit_basis"}))
        reject("setup_contract_invalid",
               "reviewed requirement entry is invalid");
      ...
      if ((quantity != "displacement" && quantity != "von_mises_stress" &&
        ...
        reject("setup_contract_invalid",
               "reviewed requirement enum is invalid");
      setup.requirements.push_back(
          {.quantity = quantity == "displacement"
           ...
           .source_or_exploratory_rationale = json_string(
               requirement, "source_or_exploratory_rationale"),
           .reviewed = json_bool(requirement, "reviewed"),
           .limit_basis = json_string(requirement, "limit_basis")});
    }
  }
  setup.mesh_controls = {
  ...
  setup.evidence_schema_version = version;
  return setup;
}
