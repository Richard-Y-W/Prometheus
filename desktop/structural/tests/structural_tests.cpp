...
3, 1, 3, 2, 5
          ...
          {{.quantity = ps::RequirementQuantity::displacement,
           ...
           {.quantity = ps::RequirementQuantity::von_mises_stress,
            ...
            .source_or_exploratory_rationale =
                "explicit exploratory benchmark limits",
            .reviewed = true,
            .limit_basis = "explicit exploratory stress limit"}},
      .mesh_controls =
  ...
  require(setupEvidence.at("schema_version") == "2.1.0" &&
          ...
          "new setup evidence has one typed requirement list and no parallel limit object");
  ...
  withUncovered.requirements.push_back(
      {.quantity = ps::RequirementQuantity::other,
       .other_quantity_description = "fatigue life under duty cycle",
       .comparator = ps::RequirementComparator::less_or_equal,
       .limit_value = 1.0e6,
       .unit = "cycles",
       .applicability = "bounded tetrahedral benchmark",
       .criticality = ps::RequirementCriticality::critical,
       .source_or_exploratory_rationale = "customer fatigue spec",
       .reviewed = true,
       .limit_basis = "customer fatigue spec"});
  require(ps::validate_setup(withUncovered).empty(),
          "an unsupported requirement remains explicit without blocking supported checks");
  auto duplicateQuantity = setup;
  duplicateQuantity.requirements.push_back(
      duplicateQuantity.requirements.front());
  require(hasIssue(ps::validate_setup(duplicateQuantity),
                   "requirement_duplicate_quantity"),
          "duplicate supported requirement quantities are rejected");
  auto onlyUncovered = setup;
  onlyUncovered.requirements = {withUncovered.requirements.back()};
  require(hasIssue(ps::validate_setup(onlyUncovered),
                   "requirement_unsupported_only"),
          "this capability cannot run when every requirement is uncovered");
  ...
  changedRequirement.requirements.front().limit_value *= 0.99;
