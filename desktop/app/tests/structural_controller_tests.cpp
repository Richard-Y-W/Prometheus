...
4, 2, 4, 3, 5
  ...
  require(controller.uncoveredRequirements().size() == 1 &&
              ...
              controller.reviewedInputHistory().size() == 6,
          "uncovered work and every persisted reviewed-input edge remain visible");
  const auto reviewedProject =
      prometheus::run_store::open_read_only(projectPath);
  require(reviewedProject.has_value() &&
              reviewedProject.value().execution.requirement_bindings.size() == 3U &&
              reviewedProject.value().execution.material_bindings.size() == 1U &&
              reviewedProject.value().execution.load_bindings.size() == 1U &&
              reviewedProject.value().execution.restraint_bindings.size() == 1U,
          "one explicit setup review publishes each graph edge once without running a solver");
